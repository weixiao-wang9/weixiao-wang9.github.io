...
![[Screenshot 2026-01-14 at 2.57.54 PM.png]]
...
$$L_{\text{sparsity}}(\theta, \phi) 

  

= \mathbb{E}_{x \sim \mu_X} \left[ 

  

   \sum_{k=1}^K w_k \, s(\hat{\rho}_k, \rho_k(x)) 
...
= - \mathbb{E}_{z \sim q_\phi} \big[ \ln p_\theta(x|z) \big] 
...
\mathbb{E}_{z \sim q_\phi(\cdot|x)} 

  

\left[ \ln \frac{p_\theta(x, z)}{q_\phi(z|x)} \right] 

  

= \ln p_\theta(x) - D_{KL}\!\left(q_\phi(\cdot|x) \,\|\, p_\theta(\cdot|x)\right)$$
...
= \mathbb{E}_{z \sim q_\phi(\cdot|x)} \big[ \ln p_\theta(x|z) \big] 

  

- D_{KL}(q_\phi(\cdot|x) \,\|\, p_\theta(\cdot)).$$
